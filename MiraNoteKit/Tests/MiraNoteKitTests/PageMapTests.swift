...
final class PageMapTests: XCTestCase {
    ...
    private func text(_ body: String, top: CGFloat, size: CGFloat = 17) -> CanvasItem {
        CanvasItem(
    ...
    func testElementsAreOrderedTopToBottomNotByCreation() {
        let memory = Memory(items: [text("lower", top: 400), text("upper", top: 60)])
        let (map, _) = PageMap.build(from: memory, canvasWidth: width)
    ...
    func testHandlesAreNumberedPerKind() {
        ...
        let memory = Memory(items: [text("title", top: 60), photo, text("body", top: 500)])
    ...
    func testBoxesAreTopLeftNotCentered() {
        ...
        let memory = Memory(items: [text("hello", top: 100)])
        let (map, _) = PageMap.build(from: memory, canvasWidth: width)
    ...
    func testCapAt24AndStateTheRemainder() {
        let items = (0..<30).map { text("block \($0)", top: CGFloat($0) * 50) }
